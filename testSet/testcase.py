...
class DefineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        while self.watcherstate:
            try:
                if (
                    self.at.driver.page_source.find(u'权限申请')) != -1 or (
                    self.at.driver.page_source.find('com.android.packageinstaller:id/permission_message')) != -1 or (
                    self.at.driver.page_source.find('android:id/alertTitle')) != -1 or (
                    self.at.driver.page_source.find('com.lbe.security.miui:id/permission_message')) != -1 or (
                    self.at.driver.page_source.find('miui:id/customPanel')) != -1 or (
                        self.at.driver.page_source.find('oppo:id/permission_prompt')) != -1 or (
                            self.at.driver.page_source.find(u'com.gwtsz.gts2:id/ads_imageview')) != -1:
                    ...
                    try:
                        self.at.get_by_id(
                            'com.gwtsz.gts2:id/btn_cancel').click()
                        time.sleep(2)
                    except BaseException:
                        pass
                else:
    ...
    def test_case001_open_app(self):
        ...
        try:
            if (self.at.driver.page_source.find(
                    self.at.desired_caps['appPackage'])) != -1:
                print u"打开app成功"
            else:
                print u"打开app失败，请检查"
                self.assertTrue(0)
    ...
    def test_case002_guide_page(self):
        try:
            ...
            time.sleep(2)
            print u"右滑"
            pic_compare_result = self.at.pic_comparison(
                0.2664,
                0.9083,
                0.7354,
                0.9542,
                'guidepage',
                'D:\\1\\test_pic\\refer_pic\\skip_guide_refer_pic.png')
        ...
        self.assertLessEqual(pic_compare_result, 1000, u'截图对比值超出预期则用例失败')
    ...
    def test_case005_enter_MyPage(self):
        try:
            self.at.get_by_xpath(
                "//android.widget.TextView[@text='我']").click()
            print u"打开'我'页面成功"
            time.sleep(5)
        except Exception:
    ...
    def test_case007_try_login_byPhone(self):
        try:
            ...
            self.at.get_by_id(
                "com.gwtsz.gts2:id/loginnameEditText").send_keys("13811113333")
            time.sleep(2)
            self.at.get_by_id("com.gwtsz.gts2:id/password").click()
            self.at.get_by_id(
                "com.gwtsz.gts2:id/password").send_keys("a123456")
            time.sleep(2)
    ...
    def test_case008_check_login_state(self):
        try:
            if (self.at.driver.page_source.find(u"解盘")) != - \
                    1 or (self.at.driver.page_source.find(u"解盤")) != -1:
                print u"登录成功"
            else:
                print u"登录失败，进行检查:"
                if (self.at.driver.page_source.find(
                        u"服务器无响应，请稍后再试(1201)")) != -1:
                    ...
                    self.at.get_by_id(
                        "com.gwtsz.gts2:id/action_btn_neg").click()
                    time.sleep(20)
    ...
    def test_case010_enter_chart_page(self, product_num=0):
        ...
        try:
            self.at.get_by_ids(
                'com.gwtsz.gts2:id/ll_product_item',
                global_product_num).click()
            time.sleep(5)
        except Exception:
    ...
    def test_case011_chart_action(self):
        try:
            ...
            time.sleep(1)
            if self.at.driver.page_source.find(
                    u"PBX") == -1 or self.at.driver.page_source.find(u"RSI") == -1:
                print u'切换指标失败，请检查'
    ...
    def test_case012_enter_trade_page(self):
        global global_product_num
        try:
            current_product = self.at.get_by_id(
                'com.gwtsz.gts2:id/app_title').text.replace('\n', '')
            ...
            if self.at.driver.page_source.find(u"市价") != -1:
                ...
                DefineTestCase.test_case010_enter_chart_page(
                    self, global_product_num)
    ...
    def test_case013_do_trade(self, input_lot_multiplier=1):
        try:
            lot_range_info = self.at.get_by_ids(
                'com.gwtsz.gts2:id/title_view_range',
                1).text.replace(
                '[',
                '_').replace(
                ']',
            ...
            input_lot = round(
                random.uniform(
                    float(
                        trade_lot_range[1]), float(
                        trade_lot_range[3])), 1) * input_lot_multiplier
            self.at.get_by_ids('com.gwtsz.gts2:id/number_input', 1).click()
            self.at.get_by_ids('com.gwtsz.gts2:id/number_input', 1).clear()
            self.at.get_by_ids(
                'com.gwtsz.gts2:id/number_input',
                1).send_keys(
                str(input_lot))
            print u'尝试下单' + str(input_lot) + u'手，若合适，则进行下一步'
            if self.at.driver.page_source.find(
                    "com.gwtsz.gts2:id/morder_alarm_title") == -1:
                self.at.get_by_id(
                    'com.gwtsz.gts2:id/btn_custom_confirm').click()
    ...
    def test_case014_check_trade_result(self):
        try:
            if self.at.driver.page_source.find(u"市价开仓成功") != -1:
                print u'市价开仓成功：' + self.at.get_by_id('com.gwtsz.gts2:id/res_status_content2').text + self.at.get_by_id(
                    'com.gwtsz.gts2:id/res_status_content3').text
            elif self.at.driver.page_source.find(u"市价开仓失败") != -1:
                print u'市价开仓失败：' + self.at.get_by_id('com.gwtsz.gts2:id/res_status_pro').text + self.at.get_by_id(
                    'com.gwtsz.gts2:id/res_status_content').text
                print u'返回重新下单'
                if self.at.driver.page_source.find(
                        u"保证金不足，为保证不影响您的交易，请及时补充资金(39)") != -1:
                    self.at.get_by_id(
                        'com.gwtsz.gts2:id/btn_custom_left').click()
